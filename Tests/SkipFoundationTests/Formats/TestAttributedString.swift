...
#if !SKIP && canImport(Darwin) // disabled for to reduce test count and avoid io.grpc.StatusRuntimeException: RESOURCE_EXHAUSTED: gRPC message exceeds maximum size
...
//        XCTAssertEqual(result, NSAttributedString(string: "Hello", attributes: [NSAttributedString.Key("TestInt") : 2]))
    ...
    func testCreateStringsFromCharactersWithUnicodeScalarIndexes() {
        ...
        if useModernFoundationFormatting {

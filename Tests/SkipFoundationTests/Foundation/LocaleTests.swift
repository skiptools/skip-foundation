...
//        XCTAssertEqual("巴克摩挪威文", zh_Hant.localizedString(forLanguageCode: "nb"))
    ...
    func testCommonISOCurrencyCodes() throws {
        ...
        let checkCurrencyCodes = Self.checkCurrencyCodes.keys.sorted()

        XCTAssertEqual(commonISOCurrencyCodes, checkCurrencyCodes)
    }
    ...
    func testCurrencyCodes() throws {
        for localeCode in Self.checkLocaleCodes {
            ...
            if localeCode == "en-US" {
                for (currencyCode, localizedString) in Self.checkCurrencyCodes {
                    XCTAssertEqual(localizedString, locale.localizedString(forCurrencyCode: currencyCode))
                    XCTAssertEqual(localizedString, locale.localizedString(forCurrencyCode: currencyCode.lowercased()))
                }
            } else {
                for currencyCode in Self.checkCurrencyCodes.keys {
                    let localizedStringCamelCase = locale.localizedString(forCurrencyCode: currencyCode)
                    let localizedStringLowercased = locale.localizedString(forCurrencyCode: currencyCode.lowercased())

                    XCTAssertNotNil(localizedStringCamelCase)
                    XCTAssertEqual(localizedStringCamelCase, localizedStringLowercased)
                }
            }
        }
    }

    // Dictionary of currency codes to en-US localized strings of the currency.
    static let checkCurrencyCodes: [String: String] = [
